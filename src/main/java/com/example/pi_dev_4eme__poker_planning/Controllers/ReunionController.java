...
//    @PutMapping("/ajouterReclamation/{idU}/{idR}")
    ...
    @PutMapping  ("/users/reunion")
    public ResponseEntity<String> addUserToReunionByUserIdAndUserNames(@RequestBody Reunion reunion,
                                                                       @RequestParam List<String> userNames,@RequestParam TypePriorite prioriteReunion) {
        try {
            reunionService.addUserToReunionByUserIdAndUserNames( reunion, userNames,prioriteReunion);
    ...
    public Reclamation getReclamationById(@PathVariable Long id) {
        ...
        @GetMapping("/reclamations-par-utilisateur")
        public Map<String, String> getReclamationsParUtilisateur() {
            return user.genererStatistiques();
        }
}
